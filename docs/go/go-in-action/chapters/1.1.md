...
### 1.1.4 内存管理
不当的内存管理会导致应用程序奔溃或内存泄漏，甚至导致操作系统奔溃。 Go语言拥有现代的垃圾回收器， 可以为你做那些艰苦的事情。 其他系统语言，例如C/C++, 你使用内存之前，需要分配内存片, 不用的时候还需要释放它们。 如果分配或释放其中一个失败，你都会有程序奔溃或内存泄漏。 不再使用内存片的时候，很难跟踪它们。线程和繁重的并发就变得更难了。 当你脑海中使用垃圾回收书写代码的时候, Go语言的垃圾回收增加了一点程序执行时间消耗，但是明显的降低了开发的难度。 Go语言去掉单调的编程，留下数豆的任务给会计。
