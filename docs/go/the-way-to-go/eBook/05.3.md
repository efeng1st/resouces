...
## go switch和其他语言switch的区别

## go switch和其他语言switch的区别
1. 不用显示提供break, 默认每个case执行完就break了。
2. 如果需要执行完一个case, 继续流转到下一个case, 需要在结束之前使用fallthrough。
3. case表达式可以是任意类型，但是所有case的表达式类型必须一样或者转换后的类型必须一样。
4. switch后面可以跟一个初始化语句，可优雅的进行条件判断。
5. switch语句还可以用于type-switch, 进行结果类型条件判断。

综上，go的switch灵活性还是比较大的，能实现很多灵活的条件判断。 比如对比两个数组中对应索引处的值大小，其他语言可能需要for循环搭配才能完成，而switch可以直接实现平行初始化。
