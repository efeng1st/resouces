...
Go语言有内置类型，比如int, string，还有用户自定义类型。Go语言中典型的用户自定义类型具有类型字段用于存储数据。如果你见过C语言中的结构体，Go语言中的用户自定义类型看起来类似，操作也类似。 但是类型也可以声明操作数据的方法。与其构建长的继承结构体Client, 扩展User, 扩展Entity, Go开发者构建小的类型Customer和Admin，然后将它们嵌入到一个大集合中。图1-4演示了继承和组合的区别。

![](images/1.4.jpg?raw=true)

**GO语言接口建模小的行为**
接口允许你表达类型行为。 如果某个类型的值实现了一个接口，则意味着该值具有特定的行为集。你甚至不需要声明你实现了一个接口；你只需要写实现。其他语言称之为鸭子模式--如果叫起来像鸭子，那么它就可以是一个鸭子--Go语言这样做很好。 在Go语言中，如果你的类型实现了接口的方法，那么你类型的值可以被哪个接口类型值存储。无需特殊声明。


在严格的面相对象语言中，比如Java,接口是无所不包的。在你甚至剋月开始编码之前，你经常需要通过一个大的继承链来进行思考。下面是一个Java接口的示例:
```
interface User {
    public void login();
    public void logout();
...
在Java中要实现这个接口, 你需要创建一个类满足所有的User接口作出的承诺和明确声明你实现这个接口。相比而言，Go语言接口一般代表一个单独的行为。 在Go语言中, 你将使用的最常见的接口是io.Reader。 io.Reader接口提供了一个简单的方法，声明你的类型以标准库中的其他函数理解的方式读取数据。下面是定义:
```
type Reader interface {
    Read(p []byte) (n int, err error)
}
```
要写一个实现io.Reader接口的类型，你只需要实现一个Read方法，接受一个字节分片，并返回一个整数和可能的错误即可。

这与其他面相对象编程语言中使用的接口系统完全背离。 Go语言的接口更小，更符合单一职责。 在实践中，这使得代码复用和可组合性的显著优势。您可以实现io.Reader在几乎任意类型具有数据可用， 然后传递给任意Go函数知道如何从io.Reader读取。 Go的整个网络库都是使用io.Reader接口构建，因为它允许将不同的网络操作所需的网络实现与应用程序的功能分开。它使接口有趣、优雅、灵活。 同样的io.Reader允许简单的文件、缓冲区、套接字以及其他数据源的操作。 使用单个接口允许你有效的进行数据操作，而不用管源如何。


### 1.1.4 内存管理
不当的内存管理会导致应用程序奔溃或内存泄漏，甚至导致操作系统奔溃。 Go语言拥有现代的垃圾回收器， 可以为你做那些艰苦的事情。 其他系统语言，例如C/C++, 你使用内存之前，需要分配内存片, 不用的时候还需要释放它们。 如果分配或释放其中一个失败，你都会有程序奔溃或内存泄漏。 不再使用内存片的时候，很难跟踪它们。线程和繁重的并发就变得更难了。 当你脑海中使用垃圾回收书写代码的时候, Go语言的垃圾回收增加了一点程序执行时间消耗，但是明显的降低了开发的难度。 Go语言去掉单调的编程，留下数豆的任务给计数器。
...
package main
import "fmt" // The import statement allows you to use external code. The fmt package provided by the standard library allows you to format and output data.
func main(){ // The main function is what gets executed when you run your application—just like in C.
...
You can even change the code to make the greeting text output in a different lan- guage. Go ahead and change the greeting inside the fmt.Println() function and hit Run again.
